...
`opposite_connected_land_3d.tscn` 是无水流的双边连通样例：北、南边为完整
`LAND`，东、西边为 `EMPTY`，两条土地边在中央属于同一个
`north_south_field`。它保留标准 `Water/RiverBed` 与
`Water/AnimatedSurface` 层级，但两者不含水几何；生长与枯萎植物层各自持有
同一可替换的 `PlantOwnerMarker` 材质。其静态规则在
`art/topologies/opposite_connected_land_3d.tres` 中显式声明为
`CENTER_CONNECTED`，土地网格由
`tools/build_opposite_connected_land_mesh.gd` 烘焙，基础校验会检查边口、中心
连续性、空边未占用以及无水层约束。

观察场景为 `scenes/visual_studies/opposite_connected_land_3d_study.tscn`，运行
后可按 `1 / 2 / 3` 切换视角，按 `B / G / W` 查看裸土、生长和枯萎层。

可用 `godot --headless --path . --script
res://tools/smoke_opposite_connected_land_runtime.gd` 再做一次入树运行时冒烟，
确认预制件进入场景树后仍能切换裸土、生长和枯萎层。

---

`north_east_land_south_water_3d.tscn` 是第一张可直接进入游戏目录的正式混合地块：北、东为
完整 `LAND`，南边中心为唯一 `WATER` 口，西边为 `EMPTY`。北东沃土在内部显式连成一个
`CENTER_CONNECTED` 区域；水道从南边直指地块中心，在最先接触该沃土内缘处停止。它引用
编辑期生成的固定土地／河岸／水网格和带 `d/s` UV2 的水面，而非运行时按边口重绘。

该地块的 `GrowingPlants` 和 `WitheredPlants` 分别引用同一套固定位置的花卉、草本丛层；裸土
