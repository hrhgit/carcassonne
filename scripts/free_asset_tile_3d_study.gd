...
func _capture() -> void:
	...
	var output_path := ProjectSettings.globalize_path("res://artifacts/free_asset_north_east_land_south_water_3d_%s_p%d.png" % [state_name, player_index + 1])
	var image := get_viewport().get_texture().get_image()
	if image == null or image.save_png(output_path) != OK:
		push_error("Could not capture Blender terrain pilot with the active display driver.")
		get_tree().quit(1)
		return
	print("FREE_ASSET_TILE_CAPTURE: %s" % output_path)
	get_tree().quit()
